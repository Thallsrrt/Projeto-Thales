#!/bin/bash
# Data de Criação: 10/05/2017
# Data de Modificação: 07:48
# Autores...
clear
...
function ADRE(){
NOME=$(dialog --stdout --inputbox "Coloque o Nome da nova rede: (obs: eth1; eth2" 0 0)
if [[ $? == '1' ]]; then
	GRES
fi
IP=$(dialog --stdout --inputbox "Coloque o IP da nova rede:" 0 0)
if [[ $? == '1' ]]; then
	GRES
fi
NETM=$(dialog --stdout --inputbox "Coloque o Netmask da nova rede:" 0 0)
if [[ $? == '1' ]]; then
	GRES
fi
NETW=$(dialog --stdout --inputbox "Coloque o Network da nova rede:" 0 0)
if [[ $? == '1' ]]; then
	GRES
fi
BROA=$(dialog --stdout --inputbox "Coloque o Broadcast da nova rede:" 0 0)
if [[ $? == '1' ]]; then
	GRES
fi

echo "########################################################################" >> /etc/network/interfaces
echo "auto $NOME" >> /etc/network/interfaces
echo "allow-hotplug $NOME" >> /etc/network/interfaces
echo "iface $NOME inet static" >> /etc/network/interfaces
...
function DPRE(){
NOME=$(dialog --stdout --title "Nome" --inputbox "Digite a rede deseja desativar : ( obs: lo; eth0; eth1" 0 0)
if [[ $? == '1' ]]; then
	GRES
fi
ifconfig $NOME down
...
function APRE(){
NOME=$(dialog --stdout --title "Nome" --inputbox "Digite a rede deseja ativar : ( obs: lo; eth0; eth1" 0 0)
if [[ $? == '1' ]]; then
	GRES
fi
ifconfig $NOME up
...
function CRMA(){
INT=$(dialog --stdout --inputbox 'Digite a rede que deseja configurar (Ex.: eth0, eth1)' 0 0)
if [[ $? == '1' ]]; then
	GRES
fi
